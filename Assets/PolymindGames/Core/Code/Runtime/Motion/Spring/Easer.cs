...
namespace PolymindGames.ProceduralMotion
{
    public static class Easer
    {
        ...
        private const float CONSTANT_D = Mathf.PI / 2.0f;
        private const float CONSTANT_E = 7.5625f;
        private const float CONSTANT_F = 2.75f;
        ...
        {
            return Mathf.Sin(13 * CONSTANT_D * p) * Mathf.Pow(2, 10 * (p - 1));
        }

        private static float ElasticOut(float p)
        {
            return Mathf.Sin(-13 * CONSTANT_D * (p + 1)) * Mathf.Pow(2, -10 * p) + 1;
        }

        private static float ElasticInOut(float p)
        {
            if (p < 0.5f)
                return 0.5f * Mathf.Sin(13 * CONSTANT_D * (2 * p)) * Mathf.Pow(2, 10 * (2 * p - 1));

            return 0.5f * (Mathf.Sin(-13 * CONSTANT_D * (2 * p - 1 + 1)) * Mathf.Pow(2, -10 * (2 * p - 1)) + 2);
        }
        ...
        {
            if (time < 1 / CONSTANT_F)
                return CONSTANT_E * time * time;

            if (time < 2 / CONSTANT_F)
                return CONSTANT_E * (time -= 1.5f / CONSTANT_F) * time + 0.75f;

            if (time < 2.5f / CONSTANT_F)
                return CONSTANT_E * (time -= 2.25f / CONSTANT_F) * time + 0.9375f;

            return CONSTANT_E * (time -= 2.625f / CONSTANT_F) * time + 0.984375f;
        }
