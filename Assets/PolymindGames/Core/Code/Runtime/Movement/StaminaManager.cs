...
namespace PolymindGames
{
    ...
    public sealed class StaminaManager : CharacterBehaviour, IStaminaManagerCC, ISaveableComponent
    {
        ...
        [SerializeField, Range(1, 1000f), EndGroup]
#if UNITY_EDITOR
        ...
        [SerializeField, Range(-1f, 25f), BeginGroup("Audio")]
        ...
        private AudioData _breathingHeavyAudio = AudioData.Default;

        private static readonly StaminaState s_DefaultState
            = new(MovementStateType.Idle, 0f, 0f, 0.2f);
        ...
        private const float HEAVY_BREATH_THRESHOLD = 0.05f;
        private const float DISABLE_MOVEMENT_THRESHOLD = 0.01f;
        private const float ENABLE_MOVEMENT_THRESHOLD = 0.2f;
        ...
        {
            if (_stamina < HEAVY_BREATH_THRESHOLD && _breathingHeavyDuration > 0f && _heavyBreathingTimer < Time.time)
            {
                Character.AudioPlayer.StartLoop(_breathingHeavyAudio.Clip, BodyPoint.Head, _breathingHeavyAudio.Volume, 1f, _breathingHeavyDuration);
        ...
        private void HandleMovementBlocking()
        {
            ...
            {
                case false when _stamina < DISABLE_MOVEMENT_THRESHOLD:
                ...
                case true when _stamina > ENABLE_MOVEMENT_THRESHOLD:
        ...
        {
            for (int i = 0; i < _staminaStates.Length; i++)
            {
                if (_staminaStates[i].StateType == stateType)
                    return _staminaStates[i];
            }
