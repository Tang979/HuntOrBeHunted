...
namespace PolymindGames.WieldableSystem
{
    ...
    public sealed class WieldableTool : Wieldable, IUseInputHandler
    {
        ...
        [SerializeField, DisableIf(nameof(_useCooldown), MIN_USE_TIME, Comparison = UnityComparisonMethod.Less)]
        [Tooltip("The UnityEvent invoked when the item is used.")]
        private UnityEvent _useEvent;

        private const float MIN_USE_TIME = 0.001f;
        ...
        {
            if (inputPhase != WieldableInputPhase.Start || !IsCrosshairActive() || _useCooldown < MIN_USE_TIME)
                return false;
