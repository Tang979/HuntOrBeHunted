...
namespace PolymindGames.ProceduralMotion
{
    internal interface ITween
    {
        ...
        #if UNITY_EDITOR
        void OnEditorValidate(GameObject gameObject);
        #endif
    }
    ...
    public abstract class Tween<DriverValueType, ComponentType> : Tween<DriverValueType> where ComponentType : Component
    {
        ...
        #if UNITY_EDITOR
        public override void OnEditorValidate(GameObject gameObject)
        {
            if (component != null || gameObject.TryGetComponent(out component))
                m_DefaultFrom = GetDefaultFrom();

            base.OnEditorValidate(gameObject);
        }
        #endif
    }
    ...
    public abstract class Tween<DriverValueType> : ITween 
    {
        ...
        [ShowIf(nameof(m_IsFromLocked), true)]
        ...
        [SerializeField, Clamp(0, 2147483647)]
...
#if UNITY_EDITOR
        public virtual void OnEditorValidate(GameObject gameObject)
        {
            if (!m_IsFromLocked)
                m_ValueFrom = default;
        }
