namespace PolymindGamesEditor.BuildingSystem
{
    ...
    public sealed class SocketEditor : ObjectEditor
    {
        ...
        private void CreatePerpendicularSocket()
        {
            ...
            Vector3 rotatedSocketPosition = RoundVector3(rotator * _socket.transform.localPosition);
            ...
            foreach (var offset in offsets)
            {
                ...
                if (s_AlignRotationToggle)
                    offset.RotationOffset = RoundVector3(rotator * offset.RotationOffset);
            }
        ...
        {
            if (CannotDisplayMesh() || _buildable == null)
            {
        ...
        private void RefreshPreviewPosition()
        {
            ...
            Quaternion rotation = transform.rotation * Quaternion.Euler(_socket.Offsets[s_SelectedOffsetIndex].RotationOffset);
        ...
        private void DoPieceOffsetTools()
        {
            ...
            EditorGUI.BeginChangeCheck();
            var handlePos = Handles.PositionHandle(pieceWorldPos, Quaternion.Euler(_socket.transform.rotation * _selectedOffset.RotationOffset));

            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(target, "Socket");

                handlePos = RoundVector3(_socket.transform.InverseTransformPoint(handlePos));
        ...
        private bool HasValidPiece() => _currentPreview != null && _offsetsList.count != 0 && s_SelectedOffsetIndex >= 0 && _selectedOffset != null && _selectedOffset.Category != 0;
        private bool CannotDisplayMesh() => !s_PreviewEnabled || Selection.activeGameObject == null || Selection.activeGameObject != _socket.gameObject;

        private Vector3 RoundVector3(Vector3 source, int digits = 3)
        {
            source.x = (float)Math.Round(source.x, digits);
            if (Mathf.Approximately(source.x, 0f))
                source.x = 0f;

            source.y = (float)Math.Round(source.y, digits);
            if (Mathf.Approximately(source.y, 0f))
                source.y = 0f;

            source.z = (float)Math.Round(source.z, digits);
            if (Mathf.Approximately(source.y, 0f))
                source.y = 0f;
