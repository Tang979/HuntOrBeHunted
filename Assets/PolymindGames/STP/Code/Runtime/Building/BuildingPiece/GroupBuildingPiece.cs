...
namespace PolymindGames.BuildingSystem
{
    ...
    {
        [SerializeField, BeginGroup("Sockets")]
        ...
        public override Vector3 GetCenter()
        {
            ...
            foreach (var piece in _parentGroup.BuildingPieces)
            {
                ...
                {
                    center += piece.GetWorldBounds().center.GetHorizontal();
                    notBuildCount++;
                }
            }
            ...
            var parenGroupBounds = _parentGroup.GetWorldBounds();
            return new Vector3(center.x, center.y - parenGroupBounds.extents.y, center.z);
        }
        ...
        {
            if (_parentGroup != null && UnityUtils.IsPlayMode)
            {
            ...
            foreach (var piece in _parentGroup.BuildingPieces)
            {
                ...
                foreach (var socket in piece.GetSockets())
                {
                    ...
                    foreach (var baseSocket in GetSockets())
                    {
                        ...
                        const float MIN_DISTANCE_BETWEEN_SOCKETS = 0.125f;
                        if (Vector3.Distance(socket.WorldPosition, baseSocket.WorldPosition) < MIN_DISTANCE_BETWEEN_SOCKETS)
                        {
