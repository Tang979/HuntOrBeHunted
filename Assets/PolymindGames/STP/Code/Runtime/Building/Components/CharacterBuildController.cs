...
namespace PolymindGames.BuildingSystem
{
    ...
    public sealed class CharacterBuildController : CharacterBehaviour, IBuildControllerCC
    {
        ...
        {
            var nextPiece = createNew ? Instantiate(_buildingPiece.Definition.Prefab) : null;
            _buildingPiece = null;
            SetBuildingPiece(nextPiece);

            // Play place effects
            ShakeEvents.DoShake(transform.position, _effectSettings.CameraShake, 5f);
        ...
        // Method to handle failed placement
        private void HandleFailedPlacement()
        {
            Character.AudioPlayer.PlaySafe(_effectSettings.InvalidPlaceAudio, BodyPoint.Torso);
        }
        ...
        private (Vector3, Quaternion) GetFreePlacement(out bool hasPlacementSurface)
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hitInfo, _buildSettings.BuildRange, placementMask, QueryTriggerInteraction.Ignore))
            {
                hasPlacementSurface = true;
                return (hitInfo.point, GetFreeRotation());
            }
            ...
            if (Physics.Raycast(startPos, Vector3.down, out var hit, 1f, placementMask, QueryTriggerInteraction.Ignore))
            {
        ...
        private Socket FindValidSocket()
        {
            ...
            int size = PhysicsUtils.OverlapSphereOptimized(headPosition, _buildSettings.BuildRange, out var colliders, LayerConstants.STRUCTURE_MASK);

            float closestSocketAngle = Mathf.Infinity;
        ...
        private struct EffectSettings
        {
            ...
            public AudioDataSO InvalidPlaceAudio;
        }
