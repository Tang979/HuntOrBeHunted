...
namespace PolymindGames.OdinSerializer
{
    ...
    public sealed class JsonDataWriter : BaseDataWriter
    {
        private static readonly uint[] s_ByteToHexCharLookup = CreateByteToHexLookup();
        private static readonly string s_NewLine = Environment.NewLine;

        private bool m_JustStarted;
        private bool m_ForceNoSeparatorNextLine;

        //private StringBuilder escapeStringBuilder;
        //private StreamWriter writer;

        private readonly Dictionary<Type, Delegate> m_PrimitiveTypeWriters;
        private readonly Dictionary<Type, int> m_SeenTypes = new(16);

        private readonly byte[] m_Buffer = new byte[1024 * 100];
        ...
        public JsonDataWriter(Stream stream, SerializationContext context, bool formatAsReadable = true) : base(stream, context)
        {
            ...
            m_PrimitiveTypeWriters = new Dictionary<Type, Delegate>()
            {
        ...
        public override void BeginReferenceNode(string name, Type type, int id)
        {
            ...
            m_ForceNoSeparatorNextLine = true;
        ...
        public override void WritePrimitiveArray<T>(T[] array)
        {
            ...
            Action<string, T> writer = (Action<string, T>)m_PrimitiveTypeWriters[typeof(T)];
        ...
        public override void WriteString(string name, string value)
        {
            ...
            if (name != null)
            {
                ...
                {
                    m_Buffer[m_BufferIndex++] = (byte)name[i];
                }
        ...
        private void WriteEntry(string name, string contents)
        {
            ...
            {
                m_Buffer[m_BufferIndex++] = (byte)contents[i];
            }
        }
        ...
        private void WriteEntry(string name, string contents, char surroundContentsWith)
        {
            ...
            m_Buffer[m_BufferIndex++] = (byte)surroundContentsWith;
        ...
        private void WriteTypeEntry(Type type)
        {
            ...
            {
                if (m_SeenTypes.TryGetValue(type, out id))
                {
                    WriteInt32(JsonConfig.TYPE_SIG, id);
                }
                else
                {
                    id = m_SeenTypes.Count;
                    m_SeenTypes.Add(type, id);
        ...
        private void StartNewLine(bool noSeparator = false)
        {
            ...
            if (noSeparator == false && m_ForceNoSeparatorNextLine == false)
            {
                EnsureBufferSpace(1);
                m_Buffer[m_BufferIndex++] = (byte)',';
            }

            m_ForceNoSeparatorNextLine = false;

            if (FormatAsReadable)
            {
                int count = NodeDepth * 4;

                EnsureBufferSpace(s_NewLine.Length + count);

                for (int i = 0; i < s_NewLine.Length; i++)
                {
                    m_Buffer[m_BufferIndex++] = (byte)s_NewLine[i];
                }
        ...
        private void Buffer_WriteString_WithEscape(string str)
        {
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                if (c < 0 || c > 127)
                {
                    ...
                    m_Buffer[m_BufferIndex++] = (byte)'\\';
                    m_Buffer[m_BufferIndex++] = (byte)'u';

                    var byte1 = c >> 8;
                    var byte2 = (byte)c;

                    var lookup = s_ByteToHexCharLookup[byte1];

                    m_Buffer[m_BufferIndex++] = (byte)lookup;
                    m_Buffer[m_BufferIndex++] = (byte)(lookup >> 16);

                    lookup = s_ByteToHexCharLookup[byte2];
                ...
                switch (c)
                {
                    ...
                    case '\f':
                        m_Buffer[m_BufferIndex++] = (byte)'\\';
                        m_Buffer[m_BufferIndex++] = (byte)'f';
                        break;

                    case '\n':
                        m_Buffer[m_BufferIndex++] = (byte)'\\';
                        m_Buffer[m_BufferIndex++] = (byte)'n';
                        break;

                    case '\r':
                        m_Buffer[m_BufferIndex++] = (byte)'\\';
                        m_Buffer[m_BufferIndex++] = (byte)'r';
                        break;

                    case '\t':
                        m_Buffer[m_BufferIndex++] = (byte)'\\';
                        m_Buffer[m_BufferIndex++] = (byte)'t';
                        break;

                    case '\0':
                        m_Buffer[m_BufferIndex++] = (byte)'\\';
                        m_Buffer[m_BufferIndex++] = (byte)'0';
                        break;

                    default:
                        m_Buffer[m_BufferIndex++] = (byte)c;
                        break;
                }
            }
        }
