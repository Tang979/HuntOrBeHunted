...
namespace PolymindGames.OdinSerializer
{
    ...
    {
        private static readonly uint[] s_ByteToHexCharLookupLowerCase = CreateByteToHexLookup(false);
        private static readonly uint[] s_ByteToHexCharLookupUpperCase = CreateByteToHexLookup(true);

        // 16x16 table, set up for direct visual correlation to Unicode table with hex coords
        private static readonly byte[] s_HexToByteLookup = new byte[] {
        ...
        {
            var lookup = lowerCaseHexChars ? s_ByteToHexCharLookupLowerCase : s_ByteToHexCharLookupUpperCase;
        ...
        public static byte[] HexStringToBytes(string hex)
        {
            ...
            for (int i = 0; i < rLength; i++)
            {
                ...
                try
                {
                    b1 = s_HexToByteLookup[hex[offset]];
                ...
                try
                {
                    b2 = s_HexToByteLookup[hex[offset + 1]];
